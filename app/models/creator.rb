...
class Creator < ApplicationRecord
  ...
  def log_changes
    if saved_change_to_name? && !previously_new_record?
      ModAction.log!(:creator_rename, self, new_name: name, old_name: name_before_last_save)
    end
    if saved_change_to_is_locked?
      ModAction.log!(is_locked ? :creator_lock : :creator_unlock, self)
    end
    if saved_change_to_linked_user_id?
      if linked_user_id.present?
        ModAction.log!(:creator_user_link, self, user_id: linked_user_id)
      else
        ModAction.log!(:creator_user_unlink, self, user_id: linked_user_id_before_last_save)
      end
    end
  end
  ...
  module NoteMethods
    ...
    def update_wiki
      if persisted? && saved_change_to_name? && attribute_before_last_save("name").present? && WikiPage.titled(attribute_before_last_save("name"))
        # we're renaming the creator, so rename the corresponding wiki page
  ...
  module LockMethods
    ...
    def validate_user_can_edit
      ...
      if is_locked?
        errors.add(:base, "Creator is locked")
        throw(:abort)
      end
    end
  ...
  def user_not_limited
    allowed = CurrentUser.can_creator_edit_with_reason
    if allowed != true
