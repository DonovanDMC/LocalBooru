...
class CreatorUrl < ApplicationRecord
  before_validation :initialize_normalized_url, on: :create
  before_validation :normalize
  validates :url, presence: true, uniqueness: { scope: :creator_id }
  validates :url, length: { in: 1..4096 }
  validate :validate_url_format
  belongs_to :creator, touch: true # Creator class
  ...
  def self.search(params)
    ...
    q = q.attribute_matches(:creator_id, params[:creator_id])
    q = q.attribute_matches(:is_active, params[:is_active])
    q = q.attribute_matches(:url, params[:url])
    q = q.attribute_matches(:normalized_url, params[:normalized_url])

    if params[:creator_name].present?
      q = q.joins(:creator).where("creators.name = ?", params[:creator_name])
    end

    # Legacy param?
    q = q.creator_matches(params[:creator])
    q = q.url_matches(params[:url_matches])
    q = q.normalized_url_matches(params[:normalized_url_matches])

    case params[:order]
    when /\A(id|creator_id|url|normalized_url|is_active|created_at|updated_at)(?:_(asc|desc))?\z/i
      dir = $2 || :desc
  ...
  def self.creator_matches(params = {})
    return all if params.blank?
    where(creator_id: Creator.search(params).reorder(nil))
  end
  ...
  # higher priority will apear higher in the creator url list
