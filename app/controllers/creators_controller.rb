...
class CreatorsController < ApplicationController
  ...
  def index
    ...
    @creators = authorize(Creator).includes(:urls).search(search_params(Creator)).paginate(params[:page], limit: params[:limit])
  ...
  def new
    @creator = authorize(Creator.new(permitted_attributes(Creator)))
  ...
  def update
    authorize(@creator).update(permitted_attributes(@creator))
    notice(@creator.valid? ? "Creator updated" : @creator.errors.full_messages.join("; "))
    respond_with(@creator)
  end

  def destroy
    authorize(@creator).destroy
  ...
  def show_or_new
    @creator = authorize(Creator).named(params[:name])
    if @creator
      redirect_to(creator_path(@creator))
    else
