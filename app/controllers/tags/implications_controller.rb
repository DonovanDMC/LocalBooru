...
module Tags
  class ImplicationsController < ApplicationController
    ...
    def index
      @tag_implications = TagImplication.includes(:antecedent_tag, :consequent_tag).search(search_params(TagImplication)).paginate(params[:page], limit: params[:limit])
    ...
    def create
      @tag_implication_request = TagImplicationRequest.new(**permitted_attributes(TagImplication).to_h.symbolize_keys)
